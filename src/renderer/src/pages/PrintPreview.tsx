...
import { Printer, FileText, Loader2, FileSpreadsheet, CalendarDays, ZoomIn, ZoomOut } from 'lucide-react'
...
export default function PrintPreview() {
    ...
    const exportOptions = [
        ...
        { id: 'screenPdf', label: 'Save as PDF (Identical)', icon: FileText, action: handleExportScreenPdf },
    ...
    return (
        <div className="space-y-6">
            <div className="print:hidden">
                ...
                <p className="text-muted-foreground">Select a branch, then pick a pay period to preview and export.</p>
            </div>
                                        ...
                                        {periods.map((p) => {
                                            ...
                                            return (
                                                <SelectItem key={key} value={key}>
                                                    {formatDate(p.pay_period_start)} — {formatDate(p.pay_period_end)} ({p.count} payslip{p.count !== 1 ? 's' : ''})
                                                </SelectItem>
                                            )
                                        })}
                                    </SelectContent>
                ...
                <div className="space-y-6">
                        ...
                        <CardHeader className="py-4">
                            <div className="flex flex-col md:flex-row justify-between gap-4">
                                <div>
                                    ...
                                    <CardDescription>Loaded {payslips.length} payslips for formatting.</CardDescription>
                                </div>
                    ...
                    <Card className="print:border-none print:shadow-none bg-gray-50/50 print:bg-transparent overflow-auto p-4 md:p-8 flex justify-center relative">
                        ...
                        <div className="absolute top-4 right-4 flex items-center space-x-1.5 border rounded-full px-2 py-0.5 print:hidden bg-white/80 backdrop-blur shadow-sm h-8 z-10">
                            <button onClick={() => setZoomLevel(p => Math.max(p - 0.1, 0.5))} className="text-muted-foreground hover:text-foreground transition-colors p-1 rounded-full hover:bg-muted" title="Zoom Out">
                                <ZoomOut className="w-3.5 h-3.5" />
                            </button>
                            <span className="text-xs font-mono font-medium w-[4ch] text-center select-none cursor-pointer" onClick={() => setZoomLevel(1)} title="Reset Zoom">
                                {Math.round(zoomLevel * 100)}%
                            </span>
                            <button onClick={() => setZoomLevel(p => Math.min(p + 0.1, 2))} className="text-muted-foreground hover:text-foreground transition-colors p-1 rounded-full hover:bg-muted" title="Zoom In">
